...
public class ParseUtils {
   ...
   /**
    * Gets the target of the redirect page. <br>
    * <b>PRECONDITION</b>: <tt>redirect</tt> must be a Redirect.
    * 
    * @param redirect
    ...
    * 
    * @param template
    *           The title of the main Template (CANNOT BE REDIRECT), including the
    *           "Template:" prefix.
    * @param wiki
    *           The wiki object to use.
    * 
    * @return The regex, in the form
    *         (?si)\{\{(Template:)??)(XXXXX|XXXX|XXXX...).*?\}\}, where XXXX is the
   ...
   /**
    * Used to check if <tt>{{bots}}</tt> or <tt>{{robots}}</tt>, case-insensitive,
    * is present in a String.
    * 
    * @param text
    *           The String to check for <tt>{{bots}}</tt> or <tt>{{nobots}}</tt>
    * @param user
    *           The account to check for, without the "User:" prefix.
    * 
    * @return boolean True if this particular bot should be allowed to edit this
    *         page.
    * 
    */
   ...
   public static boolean allowBots(String text, String user) {
      return !text
            .matches("(?i).*?\\{\\{(nobots|bots\\|(allow=none|deny=(.*?" + user + ".*?|all)|optout=all))\\}\\}.*?");
   }
   ...
   public static void templateReplace(String template, String replacementText, String reason, Wiki wiki)
         throws IOException {
    ...
    * 
    * @return The param we parsed out or null if we didn't find a param matching
    ...
    *           namespace prefix.
    * @param redirects
    *           Specify <tt>true</tt> to incorporate redirects in this parse job
    *           for this template.
    * @param wiki
    *           The wiki object to use
    * 
    * @return The template we parsed out, in the form {{TEMPLATE|ARG1|ARG2|...}} or
    *         NULL, if we didn't find the specified template.
    * @throws IOException
    *            If network error
    */
   public static String parseTemplateFromPage(String text, String template, boolean redirects, Wiki wiki)
         throws IOException {
      return redirects ? parseFromPageRegex(text, getRedirectsAsRegex("Template:" + template, wiki))
            : parseFromPageRegex(text, "(?si)\\{\\{\\s*?(Template:)??\\s*?(" + template + ").*?\\}\\}");
   }

   /**
    * Parses out the first group of matching text, based on specified regex. Useful
    * for parsing out templates.
    * 
    * @param text
    *           Text to search
    * @param regex
    ...
    * removeTemplateParam("{{template|X=A|Y=B|Z=C}}", "Y") == {{template|X=A|Z=C}}.
    * 
    * @param template
    *           the input string
    * @param param
    ...
    * with a specfied string. Used to get the start and end position of noWiki text
    * and comments
    *
    * <i>Start</i> may be "^" which is the beginning of the text. <i>End</i> may be
    ...
    * @param end
    *           the ending string
    * @return the start and end of text contanied between no Contributed by Hunsu.
    */
   ...
   /**
    * Construct template wikitext from a map that contains template parameters and
    * their values.
    * 
    * @param map
    *           the map
    * @return the template wikitext e.g. {{example|1=Blah|2=Blah2}} Contributed by
    *         Hunsu.
    */
   ...
   /**
    * Pads all template parameters to be the same length. Useful with infoboxes.
    * 
    * @param map
   ...
   /**
    * Gets the template parameters with their value. If a parameter appears many
    * times in the template just its last value will be taken.
    * 
    * @param template
    *           wikitext that consists of only a template call e.g.
    *           {{example|1=Blah|2=Blah2}}
