...
public class CCIAnalyzer {
    ...
    * 
    * @param args
    *           the command line arguments args[0] = wiki page to read (optional)
    * @throws IOException
   ...
   public static void main(String[] args) throws IOException {
      ...
      // see
      // https://en.wikipedia.org/w/api.php?action=query&prop=revisions&revids=77350972&rvdiffto=prev
      String diffaddedbegin = "&lt;td class=&quot;diff-addedline&quot;&gt;"; // <td class="diff-addedline">
      String diffaddedend = "&lt;/td&gt;"; // </td>
      String deltabegin = "&lt;ins class=&quot;diffchange diffchange-inline&quot;&gt;"; // <ins class="diffchange
                                                                                        // diffchange-inline">
      ...
      for (int i = cci.indexOf("{{dif|"); i >= 0; i = cci.indexOf("{{dif|", ++i)) {
         ...
         String diff = fetch(
               "https://en.wikipedia.org/w/api.php?format=xml&action=query&prop=revisions&rvdiffto=prev&revids="
                     + oldid);
         // Condense deltas to avoid problems like
         // https://en.wikipedia.org/w/index.php?title=&diff=prev&oldid=486611734
         diff = diff.toLowerCase();
         diff = diff.replace(deltaend + " " + deltabegin, " ");
         // If the diff is empty (see
         // https://en.wikipedia.org/w/index.php?diff=343490272)
   ...
   /**
    * Determines whether a given delta is a major edit. A "major edit" is defined
    * as something that adds more than 9 words.
    * 
    * @param delta
    *           the delta to check
